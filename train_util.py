...
def train_cooperativegame(model, optimizer, dataset, device, args,writer_epoch,grad,grad_loss):
    """
...
#    dis_step_counter = step_counters[2]
    ...
    for (batch, (inputs, masks, labels)) in enumerate(dataset):
        ...
        for idx in range(len(rationales_list)):
            ...
            assert logits.shape[0] == labels.shape[0], f"Shape mismatch: logits {logits.shape}, labels {labels.shape}"
            assert logits.shape[
                       1] == args.num_class, f"Expected logits shape (batch_size, {args.num_class}), got {logits.shape}"
