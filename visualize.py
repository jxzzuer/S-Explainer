...
with open('./data/beer/annotations.json', 'r') as f:
...
with open("annotation_test_results.txt", "w") as f:
    with torch.no_grad():
        print("Annotation")
        ...
        for item in annotation_data:
            input_text = " ".join(item['x'])
            input_tensor = torch.tensor([word2idx[word] for word in item['x'] if word in word2idx])
            input_tensor = input_tensor.to(device)
            input_tensor = input_tensor.unsqueeze(0)

            masks = torch.ones(input_tensor.shape, dtype=torch.long).to(device)
            rationales_list, logits_list = model(input_tensor, masks)


            predicted_rationale_tokens = input_tensor[0][rationales_list[0][0][:, 1].bool()].cpu().numpy()
            predicted_rationale = " ".join([str(x) for x in predicted_rationale_tokens])

            actual_label = item['y'][0]
            f.write(f"Input: {input_text}\n")
            f.write(f"Predicted Rationale: {predicted_rationale}\n")
